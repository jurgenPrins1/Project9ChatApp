...
public class MyChatActivity extends AppCompatActivity {
    ...
    private void sendMessage(String message) {
        ...
        reference.child("Messages").child(userName).child(key).setValue(messageMap).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
           if(task.isSuccessful()){
               reference.child("Messages").child(otherName).child(key).setValue(message);
           }
            }
        });
